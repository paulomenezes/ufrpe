...
workbook.xlsx.readFile('horario_final_2017.2.xlsx').then(function() {
  // use workbook
  let schedulesJSON = [];
  const daysOfWeek = {
    DOM: 0,
  ...
  function getTime(d) {
    ...
    if (timeStart.length >= 5) timeStart = timeStart.substr(timeStart.length - 5, 5);
  ...
  for (let index = 4; index <= 1488; index++) {
    ...
    let schedule = {
      ...
      cod: workbook.worksheets[0].getCell('A' + index).value
        ? '' + workbook.worksheets[0].getCell('A' + index).value
        : '',
      departament: workbook.worksheets[0].getCell('J' + index).value
        ? '' + workbook.worksheets[0].getCell('J' + index).value
        : '',
      name: '' + workbook.worksheets[0].getCell('B' + index).value,
      period: '' + workbook.worksheets[0].getCell('H' + index).value,
      place: workbook.worksheets[0].getCell('K' + index).value
        ? '' + workbook.worksheets[0].getCell('K' + index).value
        : '',
      schedules: times,
      type: '',
      vacancies: ''
  ...
  fs.writeFile('schedules_2017.2.json', JSON.stringify(schedulesJSON, null, 4), 'utf8', null);
});
